...
function RoommateAgreement() {
    ...
    const sendAgreementToConversation = async (conversationId: string, message: string) => {
        if (!isConnected) {
            showToast("Connection Error", "Unable to connect to chat server. Please try again in a moment.", "error");
            return;
        }

        const success = await sendChatMessage(message, conversationId);
    ...
    const handleSendToExistingGroup = () => {
        ...
        sendAgreementToConversation(selectedConversationId as string, formattedAgreement);
    };
