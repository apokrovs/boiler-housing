...
import { type ApiError, ListingPublic, ListingsService, ListingUpdate, LeaseAgreementPublic } from "../../client";
...
const EditListing = ({ listing, isOpen, onClose }: EditListingProps) => {
  ...
  const [leaseAgreementDescription, setLeaseAgreementDescription] = useState<string>(
    (listing.lease_agreement?.description as string) || ""
  );
  const leaseAgreementInputRef = useRef<HTMLInputElement>(null);

  // Cast the lease agreement for proper typing
  const leaseAgreement = listing.lease_agreement as LeaseAgreementPublic | undefined;
  ...
  // Lease Agreement handlers
  const handleLeaseAgreementFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      setLeaseAgreementFile(e.target.files[0]);
    }
  };

  const handleRemoveLeaseAgreement = async () => {
    if (leaseAgreement && leaseAgreement.id) {
      try {
        await ListingsService.deleteLeaseAgreement({
          listingId: listing.id,
          agreementId: leaseAgreement.id,
          currentUser: undefined,
        });
        setLeaseAgreementDescription("");
        showToast("Success", "Lease agreement removed successfully", "success");
      } catch (error) {
        console.error("Error removing lease agreement:", error);
        showToast("Error", "Failed to remove lease agreement", "error");
      }
    }
  };

  const handleLeaseAgreementUpdate = async () => {
    ...
    } else if (leaseAgreement && leaseAgreementDescription !== leaseAgreement.description) {
      await ListingsService.updateLeaseAgreement({
        listingId: listing.id,
        agreementId: leaseAgreement.id,
        currentUser: undefined,
        requestBody: { description: leaseAgreementDescription },
      });
    }
  };
  ...
  const { register, handleSubmit, reset, formState: { isSubmitting, errors } } = useForm<ListingUpdate>({
    mode: "onBlur",
    criteriaMode: "all",
  ...
  const mutation = useMutation({
    mutationFn: async (data: ListingUpdate) => {
      ...
      await handleLeaseAgreementUpdate();
      ...
      <ModalContent as="form" onSubmit={handleSubmit(onSubmit)}>
        ...
        <ModalBody pb={6}>
          ...
          <Flex>
            ...
            <Checkbox isChecked={isSecurityDeposit} onChange={(e) => setIsSecurityDeposit(e.target.checked)} />
          </Flex>
          {isSecurityDeposit && (
            <NumberInput min={0} precision={2} step={50}>
              <NumberInputField
          ...
          <CheckboxGroup value={selectedUtilities} onChange={(values) => setSelectedUtilities(values as string[])}>
          ...
          <CheckboxGroup value={selectedAmenities} onChange={(values) => setSelectedAmenities(values as string[])}>
            <Flex direction="column" gap={2}>
                ...
                <HStack spacing={3} overflowX="auto" py={2} width="100%">
                  {existingImages.map((img) => (
                    <Box key={img.id} position="relative"
                      ...
                      <Image src={`${import.meta.env.VITE_API_URL}/uploads/${img.file_path}`}
                        alt="Property image" height="80px" width="80px" objectFit="cover" />
                      <HStack position="absolute" bottom="0" width="100%" bg="blackAlpha.600" p={1} justifyContent="space-between">
                        <Button size="xs"
                        ...
                        <IconButton aria-label="Remove image" icon={<DeleteIcon />}
                          size="xs" colorScheme="red" onClick={() => handleRemoveExistingImage(img.id)} />
                      </HStack>
                    </Box>
                  ))}
                </HStack>
                  ...
                  <HStack spacing={3} overflowX="auto" py={2} width="100%">
                    {previewUrls.map((url, index) => (
                      <Box key={index} position="relative"
                        ...
                        borderRadius="md" overflow="hidden">
                        <Image src={url} alt={`Preview ${index + 1}`} height="80px" width="80px" objectFit="cover" />
                        <HStack position="absolute" bottom="0" width="100%" bg="blackAlpha.600" p={1} justifyContent="space-between">
                          <Button size="xs"
                          ...
                          <IconButton aria-label="Remove image" icon={<DeleteIcon />}
                            size="xs" colorScheme="red" onClick={() => handleRemoveFile(index)} />
                        </HStack>
                      </Box>
                    ))}
                  </HStack>
          ...
          <FormControl mt={4}>
            ...
            {leaseAgreement && !leaseAgreementFile ? (
              <Box border="1px solid" borderColor="gray.200" p={2} borderRadius="md">
                <Text>Filename: {leaseAgreement.filename}</Text>
                <Text>Type: {leaseAgreement.file_type}</Text>
                <Text>Size: {leaseAgreement.file_size} bytes</Text>
                ...
                />
                <Button mt={2} colorScheme="red" onClick={handleRemoveLeaseAgreement}>
                  Remove Lease Agreement
                </Button>
              </Box>
            ) : (
              <>
                ...
                <Button onClick={() => leaseAgreementInputRef.current?.click()}>
                  {leaseAgreement ? "Replace Lease Agreement" : "Add Lease Agreement"}
                </Button>
                <input
                  type="file"
                  accept="application/pdf,text/plain"
                  style={{ display: "none" }}
                  onChange={handleLeaseAgreementFileChange}
