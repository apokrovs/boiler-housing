...
async def upload_lease_agreement(*,
    ...
    # Check if file type is allowed for lease agreements
    ...
    # Save the file and get the path (use a separate folder for lease agreements)
...
async def get_lease_agreements(*,
    ...
    agreements = session.exec(
        select(LeaseAgreement).where(LeaseAgreement.listing_id == listing_id)
    ).all()
